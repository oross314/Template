...
class Model:
    ...
    def save(self): 
        ...
        if self.loc in vals.index.tolist():
            ...
            vals = vals.append( df)
        #########################pickle.dump(vals, open('values.p', 'wb'))   uncomment if you want to save 
    ...
    def plot(self):
        ...
        while i > 0:
            ...
            xp =  x[i]
            yp = y[i]
            diff = np.abs(10**xp-10**yp)/np.sqrt(10**xp)
            ax[0].plot(mp, diff, c = i / x.shape[0] * np.array([1, 0, .8]), )

            ax[1].plot(mp, 10**yp, c = 'blue')
            ax[1].plot(mp, 10**xp, c = 'red')
            i-=5
        ...
        ax[0].set_ylabel('relative error/ shot noise')
